...
class HitCircle extends HitObject {
	...
	render(ctx, ctx2, t, ar, cs, od) {
		var r_as;
		let hitTimeWindow = -12 * od + 259.5;
		let obj = this;
		...
		if (td > ar) renderCircle = false;
		if (!this.hit) {
			if (td < -hitTimeWindow) renderCircle = false;
		} else {
			if (t > this.hit.frame.time) renderCircle = false;
		}
		ctx.save();
		if (this.hit && t > this.hit.frame.time && t < this.hit.frame.time + judgmentTime) {
			let dt = t - this.hit.frame.time;
			if (dt < 150) {
				ctx.globalAlpha = dt / 150.0;
			} else if (dt < 350) {
				ctx.globalAlpha = 1;
			} else {
				ctx.globalAlpha = (judgmentTime - dt) / 150.0;
			}
			let dy = (dt < 100) ? (25 * (dt / 100.0)) : 25;
			ctx.drawImage(Skin.resources["hit" + this.hit.judgment], x, y - dy, cs, cs);
		}
		if (renderCircle) {
			if (td > 0) { // before circle
