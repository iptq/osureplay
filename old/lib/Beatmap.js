	...
	static ParseString(data) {
		...
		for (i = 0; i < sections.HitObjects.length; i += 1) {
			...
			if (i === 0 || hitObject.newCombo) {
				comboNumber = 1;
				comboColor = (comboColor + 1 + (hitObject instanceof HitObject.Spinner ? hitObject.customColor : 0)) % beatmap.ComboColors.length;
			} else {
				comboNumber += 1;
				if (comboNumber > beatmap.maxCombo) beatmap.maxCombo = comboNumber;
			}
