...
class HitCircle extends HitObject {
	...
	timelineRender() {
		...
		var cs = height * 0.8;
		var x = ((this.startTime / 1000 - Editor.source.t) * Editor.options.timelinezoom / 8 + .5) * width - cs / 2;
		if (x < -cs || x > width) return;
		var y = height / 5;
		ctx.drawImage(Editor.skin['hitcircle' + colIndex], x, y, cs, cs);
		var drawOverlay = function () {
			ctx.drawImage(Editor.skin.hitcircleoverlay, x, y, cs, cs);
...
class Slider extends HitObject {
		...
		if (td <= 0 && this.duration / 1000 + td > -0.2) {
			...
			while (j != jend) {
				...
				if (dpart > distance) {
					...
					ctx2.save();
					var r_fs = td > -0.1 ? cs * (1 - td * 10) : (td < -this.duration / 1000 ? 2 * cs + (td + this.duration / 1000) * 5 * 0.5 * cs : 2 * cs);
	...
	timelineRender() {
		...
		var cs = height * 0.8;
		var x = ((this.startTime / 1000 - Editor.source.t) * Editor.options.timelinezoom / 8 + .5) * width - cs / 2;
		var x2 = ((this.endTime / 1000 - Editor.source.t) * Editor.options.timelinezoom / 8 + .5) * width - cs / 2;
		if (x2 <-cs || x > width) return;
		var y = height / 5;
